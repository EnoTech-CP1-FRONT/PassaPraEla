...
app.post("/jogadoras", upload.array('imagens', 15), async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, message: `Sucesso! ${nomes.length} jogadora(s) cadastrada(s).` });
  } catch (error) {
...
(async () => {
  try {
    ...
    await db.exec(`CREATE TABLE IF NOT EXISTS usuarios (id INTEGER PRIMARY KEY, email TEXT UNIQUE, senha TEXT)`);
    await db.exec(`CREATE TABLE IF NOT EXISTS jogadoras (id INTEGER PRIMARY KEY, nome TEXT, numero_camisa INTEGER, posicao TEXT, url_imagem TEXT)`);
