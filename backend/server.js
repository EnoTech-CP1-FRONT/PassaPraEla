...
app.post("/jogadoras", upload.array("imagens", 15), async (req, res) => {
  ...
  const numeros = Array.isArray(numero_camisa) ? numero_camisa : [numero_camisa];
...
app.put("/jogadoras/:id/stats", async (req, res) => {
  ...
  try {
    await db.run(
      ...
      [gols, assistencias, finalizacoes, desarmes, defesas, gol_sofrido, cartao_amarelo, cartao_vermelho, pontuacao, id]
    );
    res.status(200).json({ success: true, message: "Pontuação atualizada com sucesso!", novaPontuacao: pontuacao });
  } catch (error) {
...
app.post("/cadastrar", async (req, res) => {
  ...
  if (!email || !senha || !nomeDaEquipe) {
    return res.status(400).json({ message: "Email, senha e nome do time são obrigatórios." });
  }
  try {
    ...
    await db.run("INSERT INTO usuarios (email, senha, nome_time) VALUES (?, ?, ?)", [email, hashedPassword, nomeDaEquipe]);
...
app.post("/login", async (req, res) => {
  ...
  if (email === ADMIN_EMAIL && senha === ADMIN_SENHA) {
    return res.status(200).json({ success: true, message: "Login de admin bem-sucedido!", redirectTo: "/admin" });
  }
  ...
  try {
    ...
    if (isMatch) {
      res.status(200).json({ success: true, message: "Login bem-sucedido!", teamName: user.nome_time, redirectTo: "/team" });
    } else {
...
app.post("/escalacao", async (req, res) => {
    ...
    try {
        ...
        await db.run('UPDATE usuarios SET escalacao_atual = ? WHERE email = ?', [escalacaoJSON, email]);
        res.status(200).json({ success: true, message: "Escalação salva com sucesso!" });
    } catch (error) {
...
app.get('/ranking', async (req, res) => {
    try {
        const ranking = await db.all('SELECT nome_time, pontuacao_total FROM usuarios ORDER BY pontuacao_total DESC LIMIT 10');
