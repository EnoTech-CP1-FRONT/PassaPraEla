...
function TeamsPage() {
  ...
  const handleSlotClick = (posicao) => {
    if (mercadoAberto) {
      ...
      alert("O mercado está fechado! Você não pode alterar sua escalação.");
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-900 font-sans">
        <main className="max-w-full mx-auto p-8 bg-white">
          ...
          <h3 className="text-2xl font-semibold text-gray-500 mb-6">Sua Escalação</h3>
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            <div className="w-full lg:w-2/3 relative">
              ...
              <img src={fieldImage} alt="Campo de futebol" className="w-full h-auto" />
              {positions.map((pos) => {
                const jogadora = team[pos.key];
                const slotClasses = `absolute text-center transition-opacity ${!mercadoAberto ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`;
                return (
                  <div key={pos.key} className={slotClasses} style={{ top: pos.top, left: pos.left, transform: "translate(-50%, -50%)" }} onClick={() => handleSlotClick(pos.key)}>
                    {jogadora ? (
                      <>
                        <img src={"http://localhost:3001" + jogadora.url_imagem} alt={jogadora.nome} className="rounded-full h-16 w-16 object-cover border-2" />
                        <span className="bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded">{pos.label}</span>
                      </>
                    ) : (
                      <>
            ...
            <div className="w-full lg:w-1/3 bg-gray-100 p-6 rounded-lg text-black">
              <ul className="space-y-3 mb-6">
                {positions.map((pos) => (
                  <li key={pos.key} className="flex justify-between items-center text-lg">
                    <span><span className="font-bold">{pos.label}</span> - {team[pos.key]?.nome || "Vazio"}</span>
                    <span className="font-bold text-green-400">{team[pos.key]?.pontuacao.toFixed(2) || "0.00"}</span>
                  </li>
                ))}
              </ul>
              <div className="border-t border-gray-600 pt-4 mb-6">
                <div className="flex justify-between text-lg font-bold">
                  <span>PONTOS:</span>
                  <span>{positions.reduce((acc, pos) => acc + (team[pos.key]?.pontuacao || 0), 0).toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
