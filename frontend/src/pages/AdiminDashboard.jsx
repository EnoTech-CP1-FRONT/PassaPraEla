...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const jogadorasResponse = await fetch("http://localhost:3001/jogadoras");
        if (!jogadorasResponse.ok) throw new Error("Falha ao buscar jogadoras.");
        const jogadorasData = await jogadorasResponse.json();
        setJogadoras(jogadorasData);

        const mercadoResponse = await fetch("http://localhost:3001/mercado/status");
        if (!mercadoResponse.ok) throw new Error("Falha ao buscar status do mercado.");
  ...
  const handleSaveStats = async (jogadoraId) => {
    ...
    const statsData = {
      gols: jogadora.gols || 0, assistencias: jogadora.assistencias || 0,
      finalizacoes: jogadora.finalizacoes || 0, desarmes: jogadora.desarmes || 0,
      defesas: jogadora.defesas || 0, gol_sofrido: jogadora.gol_sofrido || 0,
      cartao_amarelo: jogadora.cartao_amarelo || 0, cartao_vermelho: jogadora.cartao_vermelho || 0,
    };

    try {
      const response = await fetch(`http://localhost:3001/jogadoras/${jogadoraId}/stats`, {
          method: "PUT",
      ...
      setJogadoras((prev) => prev.map((j) => j.id === jogadoraId ? { ...j, pontuacao: result.novaPontuacao } : j));
  ...
  const handleToggleMercado = async () => {
      ...
      const acao = novoStatus === 'fechado' 
        ? "FLUXO DE RODADA (PASSO 1 de 2):\n\nFechar o mercado irá TRAVAR as escalações de todos os usuários. Você poderá então atualizar as pontuações.\n\nDeseja continuar?"
        : "FLUXO DE RODADA (PASSO 2 de 2):\n\nAbrir o mercado irá APURAR a pontuação final da rodada, ATUALIZAR o ranking e ZERAR tudo para a próxima rodada. Esta ação é irreversível.\n\nDeseja continuar?";
      
      // Exibe a janela de confirmação do navegador
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      ...
      <div className="mb-8 p-6 bg-white rounded-lg shadow-md">
          ...
          <p className="mb-4">O mercado está atualmente: <span className="font-bold">{mercadoStatus.toUpperCase()}</span></p>
          <button
              onClick={handleToggleMercado}
              className={`py-2 px-6 rounded-lg text-white font-bold ${mercadoStatus === 'aberto' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}
          >
              {mercadoStatus === 'aberto' ? '1. FECHAR MERCADO' : '2. APURAR PONTOS E ABRIR MERCADO'}
          </button>
      </div>
          ...
          <div key={jogadora.id} className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                <img src={`http://localhost:3001${jogadora.url_imagem}`} alt={jogadora.nome} className="w-16 h-16 rounded-full object-cover"/>
                <div>
              ...
              <div className="text-right">
                ...
                <p className="text-3xl font-bold text-purple-600">{(jogadora.pontuacao || 0).toFixed(2)}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
              {statFields.map((field) => (
                <div key={field.name}>
                  ...
                  <input type="number" value={jogadora[field.name] || 0} onChange={(e) => handleStatChange(jogadora.id, field.name, e.target.value)} className="mt-1 w-full p-2 border rounded-md"/>
                </div>
              ))}
            </div>
            ...
            {mercadoStatus === 'aberto' && <p className="text-sm text-gray-500 mt-2">Você só pode salvar a pontuação com o mercado fechado.</p>}
          </div>
        ))}
      </div>
    </div>
