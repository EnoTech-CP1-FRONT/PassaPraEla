...
export default function FormularioCompleto({ adress }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!userResponse.ok) {
        alert(`Erro no cadastro de usuário: ${userData.message}`);
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex justify-center items-center">
        <button type="submit" className="bg-purple-600 text-white px-20 py-3 rounded-md hover:bg-purple-700 mt-5">
          Finalizar Cadastro
        </button>
      </div>
    </form>
